import { IExercise, ISerie, ISerieUpdate } from "balanced-gym-model";
...
export const newSerie = (exerciseId: string, suggestedSerie: ISerie) => (
  dispatch: any
) => {
  dispatch(newSerieBegin());
  const body: ISerieUpdate = {
  ...
  return fetch(`${process.env.REACT_APP_API}/newSerie/${exerciseId}`, {
    ...
    .catch(error => dispatch(newSerieFailure(error)));
};

export const editSerie = (exerciseId: string, serie: ISerie) => (
  dispatch: any
) => {
  ...
  return fetch(
    ...
    .catch(error => {
      ...
      dispatch(editSerieFailure(error.message));
    });
};

export const deleteSerie = (exerciseId: string, serieId: string) => (
  dispatch: any
) => {
  ...
  return fetch(
    ...
    .catch(error => dispatch(deleteSerieFailure(error)));
};
